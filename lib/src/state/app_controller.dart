...
class AppController extends ChangeNotifier {
  ...
  final Set<String> _loadingSeriesCategoryIds = <String>{};
  ...
  List<LibraryEntry> get continueWatching {
    ...
    final output = <LibraryEntry>[];
    for (final entry in visibleLibrary(recent, '')) {
      if (entry.durationSeconds <= 0) continue;
      final progress = entry.progress;
      if (progress <= 0 || progress >= 0.95) continue;

      // Continue Watching should show a series once, resuming the most
      // recently watched unfinished episode for that show.
      if (entry.kind == PlaybackKind.episode) {
        final key = entry.title.trim().toLowerCase();
        if (key.isNotEmpty && !seenSeries.add(key)) continue;
      }
      output.add(entry);
    }
    return output;
  ...
  LibraryEntry? playbackHistory(PlaybackItem item) {
  ...
  bool isWatched(PlaybackItem item) {
  ...
  double playbackProgress(PlaybackItem item) {
    final entry = playbackHistory(item);
  ...
  String _seriesFavoriteId(SeriesItem item) =>
      _scopedContentId('series', item.id);

  bool isSeriesFavorite(SeriesItem item) =>
      _favoriteIds.contains(_seriesFavoriteId(item));

  Future<void> toggleSeriesFavorite(SeriesItem item) async {
    final id = _seriesFavoriteId(item);
    final existing = favorites.indexWhere((entry) => entry.id == id);
  ...
  SeriesItem? seriesForFavoriteEntry(LibraryEntry entry) {
    final match = RegExp(r'\|series:(.+)
  ...
  Future<void> _loadSeries({bool showLoading = true}) async {
    ...
    try {
      ...
      // Some providers omit the category endpoint; fall back to the full
      // catalogue only in that case.
    }
    ...
    // Do not download and decode the entire Series catalogue just to render
    // the category screen. Category rows are loaded lazily as they become
    // visible/selected. This is especially important on Android devices.
  ...
  Future<void> ensureCategoryPreviewLoaded(
    ContentSection target,
    String categoryId,
  ) async {
    if (target != ContentSection.series ||
        categoryId.isEmpty ||
        categoryId == '__all__' ||
        _loadedSeriesCategoryIds.contains(categoryId) ||
        _loadingSeriesCategoryIds.contains(categoryId)) {
      return;
    }
    _loadingSeriesCategoryIds.add(categoryId);
    try {
      await _loadSeriesCategory(categoryId, showLoading: false);
    } finally {
      _loadingSeriesCategoryIds.remove(categoryId);
    }
  }
  ...
  void _resetCatalogs() {
    ...
    _loadingSeriesCategoryIds.clear();
    _epgLoaded = false;
    contentLoading = false;
    epgLoading = false;
  }
