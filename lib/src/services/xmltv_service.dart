  ...
  Future<Map<String, List<EpgProgram>>> load(String url) async {
    ...
    try {
      return await Isolate.run(
        () => _parseXmlTvBody(body),
        debugName: 'sb-player-xmltv-parse',
      );
...
final RegExp _xmlTvOffsetPattern = RegExp(r'([+-])(\d{2})(\d{2})');

Map<String, List<EpgProgram>> _parseXmlTvBody(String body) {
  ...
  for (final node in document.findAllElements('programme')) {
    ...
    final start = _parseXmlTvDateValue(node.getAttribute('start'));
    final stop = _parseXmlTvDateValue(node.getAttribute('stop'));
...
DateTime? _parseXmlTvDateValue(String? raw) {
  ...
  final offsetMatch = _xmlTvOffsetPattern.firstMatch(suffix);
  if (offsetMatch == null) {
