...
class EpgCacheService {
  ...
  static const _fileName = 'epg_cache_v1.json';
    ...
    try {
      ...
      for (final entry in rawChannels.entries) {
        ...
        for (final raw in value.whereType<Map>()) {
          try {
            final programme = EpgProgram.fromJson(Map<String, dynamic>.from(raw));
    ...
    try {
      ...
      for (final entry in channels.entries) {
        final programmes = entry.value
            .where((programme) =>
                programme.stop.isAfter(oldest) && programme.start.isBefore(newest))
