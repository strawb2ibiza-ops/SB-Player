...
function play(x){if(view==="series")return loadSeries(x);const ext=view==="live"?"ts":(x.container_extension||"mp4"),url=base()+"/"+(view==="live"?"live":"movie")+"/"+encodeURIComponent(auth.username)+"/"+encodeURIComponent(auth.password)+"/"+x.stream_id+"."+ext;openVideo(url,x.name != null ? x.name : x.title)}
