(()=>{"use strict";
const $=id=>document.getElementById(id);let auth=null,view="live",items=[],categories=[],activeCat="all",lastFocus=null;
...
function focusables(){return Array.prototype.slice.call(document.querySelectorAll("button:not([disabled]),input,.card,.cat")).filter(function(x){return x.offsetParent!==null})}
...
function render(){const q=$("search").value.trim().toLowerCase();const f=items.filter(x=>(activeCat==="all"||String(x.category_id)===activeCat)&&String(x.name != null ? x.name : (x.title != null ? x.title : "")).toLowerCase().includes(q));$("grid").innerHTML=f.slice(0,1200).map(x=>{const name=x.name != null ? x.name : (x.title != null ? x.title : "Untitled"),img=x.stream_icon != null ? x.stream_icon : (x.cover != null ? x.cover : "");return '<button class="card focusable" data-i="'+items.indexOf(x)+'">'+(img?'<img src="'+esc(img)+'" onerror="this.style.display=\'none\'">':"")+'<strong>'+esc(name)+'</strong><small>'+esc(view==="live"?"Live":view==="movies"?"Movie":"Series")+'</small></button>'}).join("")||"<p>No results.</p>"}
function play(x){if(view==="series")return loadSeries(x);var id=x.stream_id||x.num;if(!id)return;var ext=view==="live"?"m3u8":(x.container_extension||"mp4");var url=base()+"/"+(view==="live"?"live":"movie")+"/"+encodeURIComponent(auth.username)+"/"+encodeURIComponent(auth.password)+"/"+id+"."+ext;openVideo(url,x.name != null ? x.name : x.title)}
...
document.addEventListener("click",e=>{const nav=e.target.closest("[data-view]");if(nav)return loadView(nav.dataset.view);const cat=e.target.closest("[data-cat]");if(cat){activeCat=cat.dataset.cat;document.querySelectorAll(".cat").forEach(x=>x.classList.toggle("active",x===cat));render();return}const card=e.target.closest("[data-i]");if(card){e.preventDefault();return play(items[parseInt(card.dataset.i,10)])}});
document.addEventListener("keydown",e=>{const k=e.keyCode||e.which;if(k===37||k===38||k===39||k===40){e.preventDefault();moveFocus(k);return}if(k===13){e.preventDefault();activate(document.activeElement);return}if(k===461||e.key==="Backspace"||e.key==="Escape"){if(!$("player").classList.contains("hidden")){e.preventDefault();closeVideo();return}if(view!=="live"){e.preventDefault();loadView("live")}}});
